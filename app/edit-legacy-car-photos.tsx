...
import {
  fetchLegacyCarPhotos,
  addLegacyCarPhotos,
  setPrimaryLegacyCarPhoto,
  deleteLegacyCarPhoto,
  type LegacyCarPhotoRow,
} from '../services/supabase/legacyCarPhotos'
import { getSignedSpottingPhotoUrl } from '../services/supabase/storage'

export default function EditLegacyCarPhotosScreen() {
  const { legacyCarId, carTitle } = useLocalSearchParams<{ legacyCarId: string; carTitle?: string }>()
  ...
  const [photos, setPhotos] = useState<LegacyCarPhotoRow[]>([])
  ...
  const handleAddPhotos = async () => {
    ...
    try {
      await addLegacyCarPhotos(legacyCarId, userId, files)
      await loadPhotos()
    } catch (e) {
  ...
  const handleSetPrimary = async (photoId: string) => {
    ...
    setActionLoading(true)
    try {
      await setPrimaryLegacyCarPhoto(legacyCarId, photoId)
      await loadPhotos()
    } catch (e) {
  ...
  const handleDelete = (photoId: string) => {
    Alert.alert(
      'Delete photo',
      'Remove this photo from the legacy car?',
      [
        { text: 'Cancel', style: 'cancel' },
        {
          ...
          onPress: async () => {
            setActionLoading(true)
            try {
              await deleteLegacyCarPhoto(photoId)
              await loadPhotos()
            } catch (e) {
  ...
  if (!legacyCarId) {
    return (
      <View style={{ flex: 1, backgroundColor: colors.bg, justifyContent: 'center', alignItems: 'center' }}>
        <Text style={{ color: colors.text }}>Invalid legacy car</Text>
      </View>
    )
  }
